...
class iPEPS:
    ...
    def run(self, params: np.ndarray) -> np.ndarray:
        ...
        self.tensors, conv = run_ctm(
            self.tensors, sim_config.chi, conv_fun=self.compute_energy
        )
...
class iPEPS_exci(iPEPS):
    ...
    def normalize_gs(self):
        ...
        self.tensors.A._data = [
            a / np.sqrt(np.abs(nrms0[i])) for i, a in enumerate(self.tensors.A)
        ]
    ...
    def evaluate(self):
        ...
        print(
            "Energies:",
            jax.lax.stop_gradient(E[3]),
            jax.lax.stop_gradient(E[0]),
            level=0,
        )
    ...
    def compute_energy(self, tensors):
        ...
        print(
            "Normalized E:",
            jax.lax.stop_gradient(E[3]) / jax.lax.stop_gradient(nrm),
            level=2,
        )
        return E[3] / nrm
...
def init_ctm_tensors(A, Ad):
    ...
    for i in range(A.size[0]):
        for j in range(A.size[1]):
            with cur_loc(i, j):
                Cs[0][0, 0] = np.expand_dims(np.array([1.0]), axis=(1))
                Cs[1][0, 0] = np.expand_dims(np.array([1.0]), axis=(1))
                Cs[2][0, 0] = np.expand_dims(np.array([1.0]), axis=(1))
                Cs[3][0, 0] = np.expand_dims(np.array([1.0]), axis=(1))
                Ts[0][0, 0] = np.expand_dims(
                    ncon([A[0, 0], Ad[0, 0]], ([1, 2, 3, 4, -1], [1, 2, 3, 4, -2])),
                    axis=(0, 1),
                )
                Ts[1][0, 0] = np.expand_dims(
                    ncon([A[0, 0], Ad[0, 0]], ([1, 2, 3, -1, 4], [1, 2, 3, -2, 4])),
                    axis=(0, 1),
                )
                Ts[2][0, 0] = np.expand_dims(
                    ncon([A[0, 0], Ad[0, 0]], ([1, 2, -1, 3, 4], [1, 2, -2, 3, 4])),
                    axis=(0, 1),
                )
