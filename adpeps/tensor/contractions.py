...
def ncon(tensors: Iterable[TensorType], indices_key: Union[str, Iterable[Iterable[int]]], 
        **kwargs) -> TensorType:
    try:
        try:
            ...
            ixs = ctr[indices_key]
            order = None
        return st_ncon(tensors, ixs, empty_class=EmptyT, order=order, mult_method=mult, **kwargs)
    except TypeError as e:
        if isinstance(indices_key, str):
            raise e
        return st_ncon(tensors, indices_key, empty_class=EmptyT, mult_method=mult, **kwargs)
